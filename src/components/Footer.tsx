      ...
      <div className="container-px relative z-10 mx-auto max-w-7xl py-16">
        <div className="grid gap-10 lg:grid-cols-4">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="mt-5 max-w-md text-sm leading-relaxed text-neutral-400">{f.description}</p>
            ...
            <div className="mt-6 flex items-center gap-3">
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-neutral-700 text-neutral-400 transition-all duration-300 hover:border-primary-500 hover:bg-primary-500 hover:text-white hover:-translate-y-1"
                >
            ...
            <ul className="mt-5 space-y-3">
              {f.quickLinks.map((link) => (
                <li key={link.path}>
                  <Link
                    to={link.path}
                    className="group flex items-center gap-2 text-sm text-neutral-400 transition-colors hover:text-primary-400"
                  >
          ...
          <div>
            ...
            <ul className="mt-5 space-y-4">
              <li className="flex items-start gap-3 text-sm text-neutral-400">
                ...
                <a href={`mailto:${c.email}`} className="transition-colors hover:text-primary-400">{c.email}</a>
              </li>
              <li className="flex items-start gap-3 text-sm text-neutral-400">
                <Phone className="mt-0.5 h-4 w-4 flex-shrink-0 text-primary-500" />
                <a href={`tel:${c.phone}`} className="transition-colors hover:text-primary-400">{c.phone}</a>
              </li>
              <li className="flex items-start gap-3 text-sm text-neutral-400">
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-neutral-800 pt-6 sm:flex-row">
          ...
          <button
            onClick={scrollTop}
            className="group flex items-center gap-2 rounded-full border border-neutral-700 px-4 py-2 text-xs font-medium text-neutral-400 transition-all hover:border-primary-500 hover:text-primary-400"
          >
            Back to top
